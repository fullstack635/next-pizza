...
const PizzaModal = observer(() => {
	...
	const key = (`pizza:${Pizza?.id}`) as const;
	const tr = productTranslations[state.LangStore.locale][key];
	const nameTr = tr?.name ?? Pizza?.name;
	const descTr = tr?.desc ?? Pizza?.desc;
								...
								<ScrollBoxOuter>
									<ScrollBoxInner>
										<ContainerOuter>
											<ContainerInner>
												<FirstRow>
													<PizzaTitle>{nameTr}</PizzaTitle>
												</FirstRow>
												<SecondRow>
													<RowInfo>{SizeSwitch(size)}, {dough === 'trad' ? 'traditional dough' : 'thin dough'}, {(state.UiStore.currentProduct as Pizza)?.weight[dough][size as Exclude<PizzaSize, 'sm'>]} g</RowInfo>
												</SecondRow>
												<ThirdRow>
													<PizzaDescription>{descTr}</PizzaDescription>
												</ThirdRow>
												<FourthRow>
													<SizeTumblerContainer>
														<SizeTumbler sizeSwitch={() => StyleSizeSwitch(size)} />
														<Checkbox type='radio' value={1} />
														<TumblerLabel onClick={() => setSize('sm')}>Small</TumblerLabel>
														<Checkbox type='radio' value={2} />
														<TumblerLabel onClick={() => setSize('md')}>Medium</TumblerLabel>
														<Checkbox type='radio' value={3} />
														<TumblerLabel onClick={() => setSize('xl')}>Large</TumblerLabel>
													</SizeTumblerContainer>
													<DoughTumblerContainer>
														<DoughTumbler dough={dough} />
														<Checkbox />
														<TumblerLabel onClick={() => setDough('trad')}>Traditional</TumblerLabel>
														<Checkbox />
														<TumblerLabel onClick={() => setDough('thin')}>Thin</TumblerLabel>
													</DoughTumblerContainer>
												</FourthRow>
											</ContainerInner>
										</ContainerOuter>
